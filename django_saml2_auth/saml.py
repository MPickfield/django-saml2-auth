...
def get_metadata(user_id: Optional[str] = None) -> Mapping[str, Any]:
    ...
    """
    saml2_auth_settings = settings.SAML2_AUTH
    get_metadata_trigger = dictor(
        saml2_auth_settings, "TRIGGER.GET_METADATA_AUTO_CONF_URLS"
    )
...
def decode_saml_response(
    ...
    saml_client = get_saml_client(
        get_assertion_url(request), acs, saml_response=saml_response
    )
    if not saml_client:
        raise SAMLAuthError(
...
def extract_user_identity(user_identity: Dict[str, Any]) -> Dict[str, Optional[Any]]:
    ...
    user = {}
    user["email"] = dictor(
        user_identity, f"{email_field}/0", pathsep="/"
    )  # Path includes "."
